...
public class NeohaulRecipeProvider {
    public static void buildRecipes(RecipeOutput recipeOutput) { // TODO add mod loaded condition));
        ...
        new ProcessorRecipeBuilder(AssemblerRecipe.class, 1, 1).addItemInput(semiconductors.get(SemiconductorType.SILICON_N_DOPED), 1).addItemInput(tag(DUSTS, "redstone"), 4).addItemInput(tag(INGOTS, "gold"), 1).addItemInput(tag(INGOTS, "silver"), 1).addItemResult(semiconductors.get(SemiconductorType.BASIC_PROCESSOR), 1).save(recipeOutput);
