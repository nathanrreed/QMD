...
public class RingAcceleratorLogic extends AcceleratorLogic {
    ...
    public boolean isMachineWhole() {
        ...
        if (synchrotronPorts > 1) {
            multiblock.setLastError(QMD.MOD_ID + ".multiblock_validation.accelerator.ring.to_many_synchrotron_ports");
            return false;
        }
    ...
    public Set<BlockPos> getinteriorAxisConners() {
        ...
        postions.add(acc.getExtremeInteriorCoord(false, false, false).offset(1, acc.getInteriorLengthY() / 2, (getThickness() - 2) / 2));
        postions.add(acc.getExtremeInteriorCoord(false, false, true).offset(1, acc.getInteriorLengthY() / 2, -(getThickness() - 2) / 2));
        postions.add(acc.getExtremeInteriorCoord(true, false, false).offset(-(getThickness() - 2) / 2, acc.getInteriorLengthY() / 2, 1));
        postions.add(acc.getExtremeInteriorCoord(true, false, true).offset(-(getThickness() - 2) / 2, acc.getInteriorLengthY() / 2, -1));
    ...
    public void onAcceleratorFormed() {
        ...
        if (!getWorld().isClientSide()) {
            resetBeams();
            acc.beams.get(0).setMinEnergy(QMDServerConfig.minimium_accelerator_ring_input_particle_energy);
    ...
    protected void push() {
        for (TileAcceleratorSynchrotronPort port : multiblock.getPartMap(TileAcceleratorSynchrotronPort.class).values()) {
            if (port != null && port.getOutwardFacing() != null) {
                BlockEntity tile = multiblock.getWorld().getBlockEntity(port.getBlockPos().relative(port.getOutwardFacing()));
                if (tile != null) {
                    IParticleStackHandler otherStorage = getWorld().getCapability(CapabilityParticleStackHandler.BLOCK, tile.getBlockPos(), port.getOutwardFacing().getOpposite());
